...
class EDataTables extends CGridView
{
	...
	public static function restoreState($id, $prefix = 'edt_') {
		...
		foreach($_COOKIE as $key=>$value) {
			...
			//if (substr($key, -strlen($id)) !== $id) continue;
	...
	protected function restoreStateSessionInternal() {
		...
		self::restoreStateSession($this->getId(), $sort, $pagination,$this->columns);
	}
	...
	protected function initColumnsJS() {
		...
		foreach($this->columns as $i=>$column) {
			...
			if (!property_exists($column,'sortable') || !$column->sortable){
				...
				$sortedColumns[$defaultOrder[$column->name] == 'asc' ? 'asc' : 'desc'][] = $i;
				$sortedColumns['all'][] = array($i, $defaultOrder[$column->name] == 'asc' ? 'asc' : 'desc');
			}
	...
	public static function initClientScript($bootstrap=false, $fixedHeaders=false, $configurable=false){
		...
		$cs->registerScriptFile($baseScriptUrl.'/jquery.dataTables'.(YII_DEBUG ? '' : '.min' ).'.js',CClientScript::POS_END);
		if ($configurable) {
			$cs->registerScriptFile($baseScriptUrl.'/ColReorder'.(YII_DEBUG ? '' : '.min' ).'.js',CClientScript::POS_END);
			...
			$cs->registerScriptFile($selectBaseUrl . '/select2.'.(YII_DEBUG ? 'min.' : '').'js',CClientScript::POS_END);
		}
		$cs->registerScriptFile($baseScriptUrl.'/jquery.fnSetFilteringDelay.js',CClientScript::POS_END);
