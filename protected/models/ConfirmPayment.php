...
/**
 * This is the model class for table "confirm_payment".
 *
 * The followings are the available columns in table 'confirm_payment':
 * @property integer $id
...
class ConfirmPayment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('order_id, order_code, text_detail', 'required'),
			array('order_id', 'numerical', 'integerOnly'=>true),
			array('order_code', 'length', 'max'=>17),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, order_id, order_code, text_detail', 'safe', 'on'=>'search'),
		);
	}
